    ...
    public class CityGenerator
    {
        ...
        private void MakeDistricts()
        {
            ...
            foreach (District distr in fCity.Districts) {
                ...
                if (dist <= Rad14) {
                    distr.Prosperity = Prosperity.Lux;
                } else if (dist < Rad12) {
                    distr.Prosperity = Prosperity.Good;
                } else {
                    distr.Prosperity = Prosperity.Poor;
                }
        ...
        private void MakeBuildings(District distr)
        {
            ...
            IList<BSPNode> leaves = tree.Leaves;
            foreach (BSPNode node in leaves) {
                if (prosp != Prosperity.Poor && node.NodeType == LocationType.ltInner) {
        ...
        private void SetBuildingStreet(District distr, Building bld, ExtRect area)
        {
            ...
            sides.Add(new BuildingSide(SideType.Top, Math.Abs(distrArea.Top - area.Top)));
            sides.Add(new BuildingSide(SideType.Left, Math.Abs(distrArea.Left - area.Left)));
            sides.Add(new BuildingSide(SideType.Right, Math.Abs(distrArea.Right - area.Right)));
            sides.Add(new BuildingSide(SideType.Bottom, Math.Abs(distrArea.Bottom - area.Bottom)));
        ...
        private void MakeSquare(District distr)
        {
            ...
            AddStreetlight(center);
        }

        private Streetlight AddStreetlight(ExtPoint pt, int power = 20)
        {
            Streetlight res = new Streetlight(fCity.Space, fMap, pt.X, pt.Y, power);
        ...
        private void MakeGraveyard(District distr)
        {
            ...
            foreach (BSPNode node in leaves) {
                ...
                Axis axis = (arh < arw) ? Axis.Vert : Axis.Horz;
                int count = (axis == Axis.Horz) ? arh / 3 : arw / 3;
            ...
            // create streetlights
            int lightNum = RandomHelper.GetBoundedRnd(2, 7);
            for (int i = 0; i < lightNum; i++) {
                var pt = fMap.SearchFreeLocation(area, 3);
                if (!pt.IsEmpty) {
                    AddStreetlight(pt, RandomHelper.GetBoundedRnd(3, 7));
                }
            }
        }
