...
namespace MysteriesRL.Views
{
    public sealed class PlayerChoiceView : SubView
    {
        ...
        {
            LocatedEntityList<Creature> population = ((Layer)GameSpace.CurrentRealm.PlainMap).Creatures;

            for (int i = 0; i < population.Count; i++) {
                Human human = (Human)population[i];
                int age = human.Age;

                Building bld = human.Apartment;

                if (age >= 20 && age <= 35 && bld != null && bld.Status == HouseStatus.Mansion) {
